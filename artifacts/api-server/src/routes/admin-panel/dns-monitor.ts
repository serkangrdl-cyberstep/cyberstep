...
// GET /api/admin-panel/dns-monitor/changes — tüm müşterilerin DNS değişiklikleri
router.get("/admin-panel/dns-monitor/changes", requireAdmin, async (_req: Request, res: Response) => {
  try {
    const result = await db.execute(sql`
      SELECT e.id, e.customer_id, c.email AS customer_email, c.company_name,
             e.domain, e.record_type, e.old_values, e.new_values,
             e.severity, e.soc_case_id, e.detected_at
...
// GET /api/admin-panel/dns-monitor/domains — tüm izlenen domainler
router.get("/admin-panel/dns-monitor/domains", requireAdmin, async (_req: Request, res: Response) => {
  try {
    const result = await db.execute(sql`
      SELECT d.id, d.customer_id, c.email AS customer_email, c.company_name,
             d.domain, d.is_active, d.created_at, d.last_checked_at
  ...
  try {
    const result = await db.execute<{ watched: string; changes24h: string; critical24h: string }>(sql`
      SELECT
        ...
        (SELECT COUNT(*) FROM dns_change_events WHERE detected_at > NOW() - INTERVAL '24 hours' AND severity IN ('critical','high'))::text AS critical24h
    `);
