...
router.get("/admin-panel/settings/services", requireAdmin, (_req: Request, res: Response) => {
  ...
  const services = [
    ...
    // ─── Lead Keşfi — Ücretsiz ───────────────────────────────────────────
    { id: "certstream",    name: "Certstream",             category: "Lead Keşfi",              always: true,  active: true,                              cost: "free",     costLabel: "Ücretsiz",   envKey: null,                          desc: "Gerçek zamanlı SSL CT log akışı — yeni sertifika düzenlendiği anda .tr TLD'lerini yakalar", why: "Replit WebSocket proxy kısıtlaması nedeniyle doğrudan bağlantı çalışmıyor. GitHub Actions Bridge her 10 dakikada Certstream'e bağlanır, yakaladığı TR domain'leri /api/internal/cert-ingest üzerinden Replit'e iletir", how: "GitHub repo'sunda certstream-bridge.yml workflow aktif, REPLIT_INGEST_URL + BRIDGE_SECRET GitHub Secrets olarak tanımlı", setup: "GitHub Secrets'a REPLIT_INGEST_URL ve BRIDGE_SECRET ekle. REPLIT_INGEST_URL=https://{domain}/api/internal/cert-ingest formatında", docs: "https://certstream.calidog.io/" },
    { id: "github-actions-bridge", name: "GitHub Actions Bridge", category: "Lead Keşfi",          always: false, active: false,                             cost: "free",     costLabel: "Ücretsiz",   envKey: "BRIDGE_SECRET",               desc: "Certstream → GitHub Actions → Replit ingest köprüsü — her 10 dakikada çalışır, günde ~500-1000 TR domain", why: "Replit sandbox WebSocket kısıtlamasını aşar. GitHub Actions certstream-bridge.yml, wss://certstream.calidog.io'a bağlanır, .tr TLD'lerini filtreler ve /api/internal/cert-ingest'e POST gönderir", how: "GitHub Actions ücretsiz tier'da dakika başına çalışabilir. Her çalışma 100sn Certstream'i dinler, bulunan domain'leri batch olarak Replit'e gönderir", setup: "1) GitHub repo'nuzda Settings > Secrets'a REPLIT_INGEST_URL ve BRIDGE_SECRET ekle 2) .github/workflows/certstream-bridge.yml mevcut 3) BRIDGE_SECRET env var'ını da Replit Secrets'a ekle", docs: "https://github.com/features/actions" },
    { id: "netcraft",      name: "Netcraft",               category: "Lead Keşfi",              always: false, active: false,                             cost: "enterprise", costLabel: "Enterprise",      envKey: "NETCRAFT_API_KEY",           desc: "Domain risk skoru, hosting geçmişi ve teknoloji envanteri — Enterprise müşterilere özel API erişimi gerektirir", why: "crt.sh ve RIPE DNS'e bağımsız bir lead ve OSINT kaynağı olabilir. Ancak Netcraft API'si kamuya açık değil; erişim için Netcraft ile doğrudan iletişime geçilmesi gerekiyor", how: "NETCRAFT_API_KEY tanımlandığında her gece 05:30'da 5 TR TLD sorgulanır ve lead_candidates tablosuna eklenir. Key yoksa cron sessizce atlanır.", setup: "netcraft.com ile iletişime geçerek Enterprise API erişimi talep edin. Key alındıktan sonra Replit Secrets'a NETCRAFT_API_KEY olarak ekleyin.", docs: "https://www.netcraft.com/api/" },
    { id: "bgptools",      name: "RIPE ASN Bridge",        category: "Lead Keşfi",              always: true,  active: true,                              cost: "free",     costLabel: "Ücretsiz",   envKey: null,                          desc: "RIPE stat TR ASN listesi → ASN başına announced-prefix → dig PTR reverse DNS → yeni .tr domain keşfi — API key gerektirmez", why: "ripe-dns-bridge'den farklı olarak rastgele prefix değil ASN bazlı hedefli örnekleme yapar. Her şirketin kendi IP bloğundan domain keşfedilir. bgp.tools REST API'si çalışmadığından RIPE stat kullanılıyor.", how: "Her gece 01:15'te GitHub Actions (bgptools-bridge.yml) RIPE stat'tan 50 TR ASN örnekler, her ASN'den max 4 prefix alır, dig PTR ile hostname'leri bulur ve cert-ingest'e gönderir.", setup: "Kurulum gerekmez. Certstream bridge ile aynı GitHub Secrets (REPLIT_INGEST_URL + BRIDGE_SECRET) kullanılır.", docs: "https://stat.ripe.net/docs/data_api" },
    { id: "hackertarget",  name: "HackerTarget",          category: "Lead Keşfi",              always: false, active: false,                             cost: "free",     costLabel: "Ücretsiz",   envKey: null,                          desc: "Reverse DNS lookup API'si — günlük 100 istek limiti nedeniyle dig PTR ile değiştirildi, artık kullanılmıyor", why: "100 istek/gün limiti GitHub Actions bridge'lerin ihtiyacını karşılamıyor. dig PTR komutu rate limit olmadan doğrudan DNS sorgulama yapıyor.", how: "Artık aktif değil. RIPE DNS ve RIPE ASN bridge'leri dig PTR kullanıyor.", setup: "Kurulum gerekmez.", docs: "https://hackertarget.com/ip-tools/" },
    { id: "ripe-stat",     name: "RIPE stat.ripe.net",    category: "Lead Keşfi",              always: true,  active: true,                              cost: "free",     costLabel: "Ücretsiz",   envKey: null,                          desc: "RIPE NCC açık API'si — Türkiye IPv4 prefix listesi (~2000 blok) + ASN listesi (1080 TR ASN) + IP başına network-info ve WHOIS — API key gerektirmez", why: "Lead keşfinde iki bridge'in temel kaynağı (ripe-dns + ripe-asn). Domain scan OSINT zenginleştirmesinde de kullanılıyor: WAF varsa hedef IP'nin ASN, IP prefix ve organizasyon adı çekilir", how: "1) ripe-dns-bridge.yml: her gece 01:00, 300 prefix örnekle, dig PTR  2) bgptools-bridge.yml: her gece 01:15, 50 ASN × 4 prefix  3) OSINT enrichment: domain scan sırasında hedef IP için network-info + WHOIS sorgusu", setup: "Kurulum gerekmez. stat.ripe.net tamamen açık bir API'dir.", docs: "https://stat.ripe.net/docs/data_api" },
    { id: "mersis",        name: "MERSİS",                category: "Lead Keşfi",              always: true,  active: true,                              cost: "free",     costLabel: "Ücretsiz",   envKey: null,                          desc: "Ticaret Bakanlığı Merkezi Sicil Kayıt Sistemi — şirket adı, vergi no, kuruluş tarihi, faaliyet kodu (NACE), merkez adresi ve ortaklık yapısı", why: "Lead zenginleştirmede şirketin yasal kimliğini, sektörünü ve büyüklüğünü doğrular; MERSİS'te kayıtsız şirketlere yapılan satış girişimlerini önler", how: "Lead TR Enrich cron'u (günlük 07:30) domain → vergi no eşleştirmesi yaparak lead_candidates.officer_name, officer_title ve isr_notes alanlarını günceller", setup: "Kurulum gerekmez. MERSİS kamuya açık bir veri tabanıdır.", docs: "https://mersis.gtb.gov.tr" },
    { id: "kap",           name: "KAP (Kamuyu Aydınlatma)", category: "Lead Keşfi",            always: true,  active: true,                              cost: "free",     costLabel: "Ücretsiz",   envKey: null,                          desc: "Borsa İstanbul halka açık şirketlerin zorunlu açıklamaları — yıllık raporlar, yönetim kurulu değişiklikleri, özel durum açıklamaları", why: "Hedef şirketin yönetim değişikliklerini ve siber güvenlik ile ilgili özel durum bildirimlerini takip eder", how: "Lead TR Enrich cron'u KAP üzerinden son açıklamaları çeker; C-level yönetici isim ve unvanlarını isr_notes alanına yazar", setup: "Kurulum gerekmez. KAP tamamen kamuya açık bir platformdur.", docs: "https://www.kap.org.tr" },
