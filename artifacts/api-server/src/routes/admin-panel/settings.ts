...
router.get("/admin-panel/settings/services", requireAdmin, (_req: Request, res: Response) => {
  ...
  const services = [
    ...
    { id: "netcraft",      name: "Netcraft",               category: "Lead Keşfi",              always: false, active: false,                             cost: "free",     costLabel: "Ücretsiz tier",   envKey: "NETCRAFT_API_KEY",           desc: "Türk TLD veritabanı — .com.tr/.net.tr/.org.tr/.web.tr/.biz.tr domain araması, 100 sonuç/sorgu", why: "crt.sh ve RIPE DNS'e ek bağımsız bir lead kaynağı. Netcraft'ın kendi domain envanteri SSL sertifikalarından farklı kaynaklarla beslenir", how: "Her gece 05:30'da NETCRAFT_API_KEY ile 5 TR TLD'si sorgulanır. Her TLD için 100 domain alınır, lead_candidates tablosuna eklenir", setup: "NETCRAFT_API_KEY ortam değişkenini Replit Secrets'a ekle. Key yoksa cron erken çıkar, hata oluşturmaz.", docs: "https://www.netcraft.com/api/" },
    { id: "bgptools",      name: "BGP.tools",              category: "Lead Keşfi",              always: true,  active: true,                              cost: "free",     costLabel: "Ücretsiz",   envKey: null,                          desc: "TR ASN prefix örneklemesi → HackerTarget reverse DNS → yeni .tr domain keşfi — API key gerektirmez", why: "RIPE DNS discovery ile benzer yaklaşım ama farklı ASN kaynağı kullanır. bgp.tools'un ASN verisi bazen RIPE stat'tan farklı prefix'ler içerir", how: "Her gece 06:15'te ilk 20 TR ASN alınır, ASN başına max 3 prefix örneklenir (toplam 60 prefix), IP'ler HackerTarget ile hostname'e çevrilir", setup: "Kurulum gerekmez. bgp.tools tamamen açık; HackerTarget free tier kullanılır.", docs: "https://bgp.tools/" },
