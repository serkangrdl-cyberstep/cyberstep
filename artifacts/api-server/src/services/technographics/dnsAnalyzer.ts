...
export async function analyzeDNS(domain: string): Promise<TechStack[]> {
  ...
  try {
    ...
    if (txtFlat.includes("v=spf1")) {
      const spfLine = txtRecords.flat().find((t) => t.startsWith("v=spf1")) || "";
    ...
    if (dmarcFlat.includes("v=dmarc1")) {
      ...
      found.push({ category: "mail_security", vendor: "dmarc", product: `DMARC (p=${policy || "unknown"})`, confidence: 100, detectedVia: "dns", securityNote: policy === "reject" ? "Tam koruma" : policy === "quarantine" ? "Kısmi koruma" : "Koruma yok (none)", securityRisk: policy === "none" ? "high" : "low", evidence: { dmarc: dmarcFlat } });
    }
