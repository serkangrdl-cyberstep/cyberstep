...
export async function analyzeShodan(domain: string): Promise<TechStack[]> {
  ...
  try {
    ...
    const dangerousPorts: Record<number, { product: string; risk: "critical" | "high" | "medium" | "low"; note: string }> = {
      ...
      9200: { product: "Elasticsearch Açık", risk: "critical", note: "Elasticsearch herkese açık" },
    ...
    for (const portData of data.ports || []) {
      ...
      if (danger) {
        found.push({ category: "open_port", vendor: "network", product: danger.product, confidence: 100, detectedVia: "shodan", securityRisk: danger.risk, securityNote: danger.note, salesSignal: "urgent_risk", evidence: { port: portData, ip: ips[0] } });
      }
    }
